// backend/engines/lumiShadowMiniEngine.js (ESM)
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { majorToArchetypeSignal } from "../lib/majorToArchetypeSignal.js";
import { blendArchetypeScores } from "../lib/blendArchetypeScores.js";
 ...
 * }
 */
export function scoreLumiShadowMiniSuite({
  miniBank,
  answersByTest,
  luminaryCatalog,
  shadowCatalog,
  ...
  for (const [testId, testDef] of Object.entries(miniBank || {})) {
    ...
    const qById = Object.fromEntries(
      (testDef.questions || []).map((q) => [q.id, q])
    );
 ...
 * Convenience loader
 */
export function loadMiniSuiteAndCatalogs() {
  // miniBank may be in various shapes. Normalize to just the test map.
  const rawMini = safeReadJSON("data/miniTests.lumiShadow.json") || {};
  const miniBank = rawMini.tests || rawMini.miniTests || rawMini;

  const luminaryCatalog =
    safeReadJSON("data/luminary.catalog.json") ||
    safeReadJSON("data/luminaries.catalog.json") ||
    [
  ...
  const shadowCatalog =
    ...
    safeReadJSON("data/shadows.catalog.json") ||
    [
